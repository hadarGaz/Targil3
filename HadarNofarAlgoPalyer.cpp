...
AlgorithmRegistration algo_NofarKedemID("algo_NofarAndHadar",
	[] {return new HadarNofrtAlgoPalyer();}
);

void HadarNofrtAlgoPalyer::setPlayer(int player)
{
	numOfPlayer = player;
}
void HadarNofrtAlgoPalyer::init(const BoardData& board) //get pboard
{
...
void HadarNofrtAlgoPalyer::initDirArr()
{
	arr[0] = Direction::UP;
 ...
 GameMove HadarNofrtAlgoPalyer::play(const GameMove& opponentsMove)
{
	 int from_x, from_y;
	 int to_x, to_y;
	 int found = 0;
 ...
 string HadarNofrtAlgoPalyer::getName() const
{
	return "311501928";
}

int HadarNofrtAlgoPalyer::convertCharToInt(char x)
{
	if (x >= 'A' && x <= 'Z')
		return x - 'A' + 1;
	else if (x >= 'a' && x <= 'z')
...
bool HadarNofrtAlgoPalyer::calcNewMove(int from_x, int from_y, int &to_x, int& to_y)
{
	int found = 0;
	int counter = 0;
	int x_dir, y_dir;
...
void HadarNofrtAlgoPalyer::getdir(Direction d, int& _x_dir, int& _y_dir)
{
	switch (d)
	{
	case Direction::UP:
...
HadarNofrtAlgoPalyer::Direction HadarNofrtAlgoPalyer::convertToDir(const int& _x_dir, const int& _y_dir)
{
	Direction p;
...
HadarNofrtAlgoPalyer::Direction HadarNofrtAlgoPalyer::findDir(int from_x,int from_y,int& x_dir,int& y_dir)
{
	Direction d;
	// calc x_dir
...
int  HadarNofrtAlgoPalyer::calcNewSol()
{
	int found = 0;
	while (!found)
	{
...
void HadarNofrtAlgoPalyer::updateBoardAndTools(char curr, int x, int y) {
	if (curr == '1' || curr == '7') {
...
bool HadarNofrtAlgoPalyer::moveIsValid(int to_x, int to_y)
{
...
void HadarNofrtAlgoPalyer::updatePosSoldier(const int& to_x, const int& to_y)
{
...
void HadarNofrtAlgoPalyer::clearTheGame()
{
